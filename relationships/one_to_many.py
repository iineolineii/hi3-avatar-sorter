...
from ..models import Battlesuit, Skin, SkinRarity

if TYPE_CHECKING:
    from ..models import Valkyrie

ChildT = TypeVar("ChildT", bound="Valkyrie | Battlesuit | SkinRarity | Skin")

@dataclass
class OneToMany(Generic[ChildT]):
    __children: dict[int, ChildT] = field(init=False, default_factory=dict)
    ...
    def __post_init__(self) -> None:
        ...
        self.__children = dict(getattr(self, self.__children_attr))
        setattr(self, self.__children_attr, self.__children)
    ...
    def _add_child(self, child: ChildT) -> ChildT:
        ...
        if len(self.__children) >= self.__max_reserved_no:
    ...
    def _add_child_mex(self, child: ChildT) -> ChildT:
        child.no = self.__mex_child_no
        self.__children_numbers.add(child.no)
        self._update_mex(child.no)
        self.__children[child.code] = child
        return child

    def _add_child_lazy(self, child: ChildT) -> ChildT:
        child.no = len(self.__children) + 1
        self.__children[child.code] = child
        return child
    ...
    @property
    def children(self) -> dict[int, ChildT]:
